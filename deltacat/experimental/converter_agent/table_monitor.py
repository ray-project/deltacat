...
def submit_table_monitor_job(
    ...
    if cluster_cfg_file_path:
        ...
        logger.info(f"Preparing to submit job to remote cluster: {cluster_cfg_file_path}")
        ...
        logger.info(f"Preparing to submit job locally with ray init args: {ray_init_args}")
    ...
    table_monitor_script_path = os.path.join(table_monitor_script_dir, "table_monitor.py")

    logger.debug(f"Table monitor script path: {table_monitor_script_path}")
    logger.debug(f"Table monitor script exists: {os.path.exists(table_monitor_script_path)}")
    ...
    logger.debug(f"Submitting table monitor job '{job_name}' with entrypoint: {entrypoint}")
