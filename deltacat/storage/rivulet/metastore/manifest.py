...
class Manifest:
    ...
    def __hash__(self):
        ...
        return hash((frozenset(self.data_files), frozenset(self.sst_files), self.context))
...
class ManifestIO(Protocol):
    ...
    def write(self, file: OutputFile, data_files: Set[str],
              sst_files: Set[str], schema: Schema, level: TreeLevel):
        ...
...
class JsonManifestIO(ManifestIO):
    ...
    def write(self, file: OutputFile, data_files: Set[str],
        ...
        with file.create() as f:
            f.write(json.dumps({"data_files": list(data_files),
    ...
    def read(self, file: InputFile):
        ...
        level = data.get("level", 0) # TODO remove default after fixing assets in ZipperReadExample.zip
        return Manifest(set(data_files), set(sst_files), ManifestContext(schema, stream_position, level))
