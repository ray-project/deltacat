...
def commit_replace_snapshot(iceberg_table, to_be_deleted_files_list, new_position_delete_files):
    tx = iceberg_table.transaction()
    snapshot_properties = {}
    commit_uuid = uuid.uuid4()
    update_snapshot = tx.update_snapshot(snapshot_properties=snapshot_properties)
    replace_snapshot = replace(self=update_snapshot, commit_uuid=commit_uuid, using_starting_sequence=False)
...
def commit_overwrite_snapshot(iceberg_table, to_be_deleted_files_list, new_position_delete_files):
    commit_uuid = uuid.uuid4()
    with iceberg_table.transaction() as tx:
        ...
        with tx.update_snapshot().overwrite(commit_uuid=commit_uuid) as overwrite_snapshot:
