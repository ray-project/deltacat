...
def estimate_iceberg_pos_delete_additional_columns(include_columns, num_of_record_count):
    total_additional_columns_sizes = 0
    if "file_path" in include_columns:
        total_additional_columns_sizes += AVERAGE_FILE_PATH_COLUMN_SIZE_BYTES * num_of_record_count
    elif "pos" in include_columns:
        total_additional_columns_sizes += AVERAGE_POS_COLUMN_SIZE_BYTES * num_of_record_count
...
def estimate_convert_remote_option_resources(data_files, equality_delete_files):
    ...
    equality_delete_record_count = get_total_record_from_iceberg_files(equality_delete_files)
    hash_column_sizes = estimate_fixed_hash_columns(XXHASH_BYTE_PER_RECORD, data_file_record_count + equality_delete_record_count)
    pos_delete_sizes = estimate_iceberg_pos_delete_additional_columns(["file_path", "pos"], data_file_record_count + equality_delete_record_count)
...
def convert_resource_options_provider(index, files_for_each_bucket):
    ...
    data_files_list, equality_delete_files_list, position_delete_files_list = files_for_each_bucket[1]
    memory_requirement = estimate_convert_remote_option_resources(data_files_list, equality_delete_files_list)
