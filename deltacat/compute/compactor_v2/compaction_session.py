...
def _execute_compaction(
    ...
    # overriding drop duplicates
    params.drop_duplicates = False
    logger.info(f"Overriding drop duplicates={params.drop_duplicates}")

    total_hb_record_count = np.int64(0)
    total_dd_record_count = np.int64(0)
    telemetry_time_hb = 0
    telemetry_time_merge = 0

    # We only execute 3000 BSP steps at a time if drop_duplicates=False
    MAX_TO_HB = 3000 if params.drop_duplicates is False else len(uniform_deltas)
    merge_results = []

    logger.info(f"Total number of BSP steps to be executed: {MAX_TO_HB}")

    for i in range(0, len(uniform_deltas), MAX_TO_HB):
        next_index = min(len(uniform_deltas), i + MAX_TO_HB)
        (
            cur_merge_results,
            cur_total_hb_record_count,
            cur_total_dd_record_count,
            cur_telemetry_time_hb,
            cur_telemetry_time_merge,
        ) = _compact_uniform_deltas(
            params=params,
            uniform_deltas=uniform_deltas[i:next_index],
        ...
        total_hb_record_count += cur_total_hb_record_count
        total_dd_record_count += cur_total_dd_record_count
        telemetry_time_hb += cur_telemetry_time_hb
        telemetry_time_merge += cur_telemetry_time_merge
        merge_results.extend(cur_merge_results)
    ...
    for mat_result in mat_results:
        ...
        if str(mat_result.task_index) in hb_id_to_entry_indices_range:
            hb_id_to_entry_indices_range[str(mat_result.task_index)][1] = (
