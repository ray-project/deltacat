...
from deltacat.expression import (
    Expression, Reference, Literal, BinaryExpression, UnaryExpression,
    In, Between, Like
)
...
class ExpressionVisitor(ABC, Generic[C, R]):
    ...
    def __init__(self):
        ...
        if not hasattr(self, 'visit_reference') or not callable(getattr(self, 'visit_reference')):
            raise NotImplementedError("Subclasses must implement visit_reference")
        if not hasattr(self, 'visit_literal') or not callable(getattr(self, 'visit_literal')):
    ...
    def _setup_default_procedure_handlers(self):
        ...
        if not hasattr(self, '_apply_binary') or not callable(getattr(self, '_apply_binary')):
            self._apply_binary = lambda proc, left, right: proc(left, right)
        if not hasattr(self, '_apply_unary') or not callable(getattr(self, '_apply_unary')):
        ...
        if not hasattr(self, '_apply_between') or not callable(getattr(self, '_apply_between')):
            self._apply_between = lambda proc, value, lower, upper: proc(value, lower, upper)
        if not hasattr(self, '_apply_like') or not callable(getattr(self, '_apply_like')):
    ...
    def _visit_binary(self, expr: BinaryExpression, context: Optional[C] = None) -> R:
        ...
        if expr_type in self._PROCEDURES:
            return self._apply_binary(self._PROCEDURES[expr_type], left_result, right_result)
        
        try:
            return self.visit_binary_expression(expr, left_result, right_result, context)
    ...
    def _visit_between(self, expr: Between, context: Optional[C] = None) -> R:
        ...
        if "Between" in self._PROCEDURES:
            ...
            return self._apply_between(
                self._PROCEDURES["Between"], 
                value_result, lower_result, upper_result
            )
    ...
    def _visit_like(self, expr: Like, context: Optional[C] = None) -> R:
        ...
        if "Like" in self._PROCEDURES:
            ...
            return self._apply_like(
                self._PROCEDURES["Like"], 
                value_result, pattern_result
            )
    ...
    def visit_binary_expression(self, expr: BinaryExpression, left: R, right: R, 
                               context: Optional[C] = None) -> R:
        """Default fallback handler for binary expressions."""
        raise NotImplementedError(f"No handler for {type(expr).__name__}")
    
    def visit_unary_expression(self, expr: UnaryExpression, operand: R,
                              context: Optional[C] = None) -> R:
...
class DisplayVisitor(ExpressionVisitor[Expression, str]):
    ...
    def _apply_binary(self, proc, left, right):
        ...
        left_parenthesized = f"({left})" if " " in left and not (left.startswith("(") and left.endswith(")")) else left
        right_parenthesized = f"({right})" if " " in right and not (right.startswith("(") and right.endswith(")")) else right
