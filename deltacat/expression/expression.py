...
class UnaryExpression(Expression, ABC):
    ...
    def of(cls, operand: Expression | Any) -> UnaryExpression:
        ...
        """
        operand_expr = operand if isinstance(operand, Expression) else Literal(pa.scalar(operand))
...
class BinaryExpression(Expression, ABC):
    ...
    def of(cls, left: Expression | Any, right: Expression | Any) -> BinaryExpression:
        ...
        right_expr = right if isinstance(right, Expression) else Literal(pa.scalar(right))
...
class BooleanExpression(Expression, ABC):
    ...
    def and_(self, other: BooleanExpression | Any) -> BooleanExpression:
        ...
        """
        other_expr = other if isinstance(other, BooleanExpression) else Literal(pa.scalar(other))
...
class Reference(Expression):
    ...
    def in_(self, values: List[Expression | Any]) -> In:
        ...
        """
        expr_values = [v if isinstance(v, Expression) else Literal(pa.scalar(v)) for v in values]
    ...
    def between(self, lower: Expression | Any, upper: Expression | Any) -> Between:
        ...
        """
        lower_expr = lower if isinstance(lower, Expression) else Literal(pa.scalar(lower))
        upper_expr = upper if isinstance(upper, Expression) else Literal(pa.scalar(upper))
    ...
    def like(self, pattern: str | Expression) -> Like:
        ...
        """
        pattern_expr = pattern if isinstance(pattern, Expression) else Literal(pa.scalar(pattern))
...
class In(BooleanExpression):
    ...
    def of(cls, value: Expression | Any, values: List[Expression | Any]) -> In:
        ...
        Returns:
            In: A new IN expression.
        """
        value_expr = value if isinstance(value, Expression) else Literal(pa.scalar(value))
        value_exprs = [v if isinstance(v, Expression) else Literal(pa.scalar(v)) for v in values]
...
class Between(BooleanExpression):
    ...
    @classmethod
    def of(cls, value: Expression | Any, lower: Expression | Any, upper: Expression | Any) -> Between:
        """
