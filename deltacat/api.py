...
def _copy_dc(source, destination, recursive=False):
    if recursive:
...
def list(
    ...
    **kwargs,
    ...
    if not dc_url.is_deltacat_catalog_url():
        ...
        if dataset_type == DatasetType.RAY_DATASET:
            ...
            return read_deltacat(
                ...
                read_kwargs_provider=CustomReadKwargsProvider(
                    datasource_type=dc_url.datastore_type,
                    kwargs=kwargs,
                ),
            )
        else:
...
def get(
    ...
    **kwargs,
    ...
    dc_url = DeltaCatUrl(path)
    reader = DeltaCatUrlReader(dc_url)
