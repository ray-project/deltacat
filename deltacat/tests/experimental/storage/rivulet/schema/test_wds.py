...
class TestWebDataset():
    def create_tar_file(samples, tmp_path):
        tar_path = os.path.join(tmp_path, "test_dataset.tar")

        with wds.TarWriter(tar_path) as writer:
    ...
    def process_tar(tar_path):
        """
        Test that tar file processing works correctly.
