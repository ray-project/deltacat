...
def test_converter_drop_duplicates_success(
    ...
    pyicberg_scan_table_rows = tbl.scan().to_arrow().to_pydict()
    primary_key = sorted(pyicberg_scan_table_rows["primary_key"])
    remaining_value = sorted(pyicberg_scan_table_rows["pos"])
    # primary key 1, 2, 3, 4 should be unique
    assert primary_key == ["pk1", "pk2", "pk3", "pk4"]
    # Only records written by latest file remains, for primary key 1, 2, 3, 4, it will be 4, 7, 8, 9
    assert remaining_value == [4, 7, 8, 9]
