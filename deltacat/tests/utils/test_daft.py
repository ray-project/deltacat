...
class TestDaftParquetReader(unittest.TestCase):
    ...
    def test_read_from_s3_all_columns(self):
        table = daft_s3_file_to_table(self.URL, content_encoding=ContentEncoding.IDENTITY.value, content_type=ContentType.PARQUET.value)
        self.assertEqual(table.schema.names, ["a", "b"])
        self.assertEqual(table.num_rows, 100)

    def test_read_from_s3_single_column(self):
        table = daft_s3_file_to_table(self.URL, content_encoding=ContentEncoding.IDENTITY.value, content_type=ContentType.PARQUET.value, include_columns=["b"])
