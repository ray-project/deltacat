...
from deltacat.tests.storage.rivulet.test_utils import write_mvp_table, compare_mvp_table_to_scan_results, mvp_table_to_record_batches, \
    validate_with_full_scan, assert_data_file_extension_set, create_dataset_for_method
...
class TestBasicEndToEnd:
    ...
    def test_end_to_end_scan_pydict(self, ds1_field_group, ds1_dataset):
        ...
        read_records: List[Dict] = list(
            self.dataset.scan(QueryExpression()).to_pydict())  # compare all_records to ds1
    ...
    def test_end_to_end_scan_key_range(self, ds1_field_group, ds1_dataset):
        read_records_range: List[Dict] = list(
            self.dataset.scan(QueryExpression().with_primary_range(100, 500)).to_pydict())
...
class TestMultiLayerCompactionEndToEnd:
    ...
    def l0_overwrite(self, ds1_dataset):
        ...
        return self._transform_dataset(ds1_dataset, math.floor(FIXTURE_ROW_COUNT / 2), FIXTURE_ROW_COUNT,
    ...
    def l1_overwrite(self, ds1_dataset):
        ...
        return self._transform_dataset(ds1_dataset, 0, math.floor(FIXTURE_ROW_COUNT / 2),
                                       transform_name=lambda x: "overwritten")
    ...
    def _transform_dataset(self, dataset, min_index=0, max_index=FIXTURE_ROW_COUNT,
                           transform_id=lambda x: x, transform_name=lambda x: x, transform_age=lambda x: x):
        data = dataset.data
        return MvpTable({
    ...
    @pytest.fixture
    def ds1_written_uri(self, ds1_schema, ds1_dataset, l2_ignored, l0_overwrite, l1_overwrite):
    ...
    def test_end_to_end_scan(self, ds1_written_uri, ds1_schema, expected_dataset):
        ...
        read_records: List[Dict] = list(self.dataset.scan(QueryExpression()).to_pydict())
    ...
    def rewrite_at_level(self, uri: str, level: TreeLevel):
        ...
        self.manifest_io.write(output_file, manifest.data_files, manifest.sst_files, manifest.context.schema, level)
...
class TestZipperMergeEndToEnd:
    ...
    def test_end_to_end_scan(self, field_group1, field_group2, ds1_schema, ds1_dataset,
                             ds2_dataset, ds2_schema, combined_schema):
        read_records: List[Dict] = list(
