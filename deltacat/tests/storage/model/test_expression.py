...
from deltacat.storage.model.expression import (
    Reference, Literal, Equal, NotEqual, GreaterThan, LessThan,
    GreaterThanEqual, LessThanEqual, And, Or, Not, In, Between,
    Like, IsNull
)
...
class TestExpressionLibrary:
    ...
    def test_complex_expression_building(self, field_ref, field_ref2):
        ...
        expr = (
            field_ref.eq(42)
            .and_(field_ref2.gt(10))
            .or_(field_ref.is_null())
            .not_()
        )
    ...
    def test_comparison_display(self, field_ref, literal_int, display_visitor):
        ...
        assert display_visitor.visit(GreaterThan(field_ref, literal_int)) == "field1 > 42"
        assert display_visitor.visit(LessThan(field_ref, literal_int)) == "field1 < 42"
        assert display_visitor.visit(GreaterThanEqual(field_ref, literal_int)) == "field1 >= 42"
        assert display_visitor.visit(LessThanEqual(field_ref, literal_int)) == "field1 <= 42"
    ...
    def test_logical_operator_display(self, field_ref, literal_int, display_visitor):
        ...
        assert display_visitor.visit(And(eq_expr, gt_expr)) == "(field1 = 42 AND field1 > 42)"
        assert display_visitor.visit(Or(eq_expr, gt_expr)) == "(field1 = 42 OR field1 > 42)"
    ...
    def test_special_operator_display(self, field_ref, display_visitor):
        ...
        assert display_visitor.visit(Between(field_ref, lower, upper)) == "field1 BETWEEN 10 AND 20"
    ...
    def test_complex_expression_display(self, field_ref, field_ref2, display_visitor):
        ...
        assert display_visitor.visit(expr) == "NOT (((field1 = 42 AND field2 > 10) OR (field1) IS NULL))"
    ...
    def test_nested_parentheses(self, field_ref, field_ref2, display_visitor):
        ...
        assert display_visitor.visit(or_expr) == "((field1 = 1 AND field2 = 2) OR field2 = 3)"
