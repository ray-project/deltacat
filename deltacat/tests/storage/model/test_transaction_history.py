...
class TestTransactionHistory:
    ...
    def create_test_transactions(self):
        ...
        orders = pd.DataFrame({"order_id": [1, 2], "user_id": [1, 2], "product_id": [101, 102]})
        ...
        analytics = pd.DataFrame({"metric": ["page_views", "clicks"], "value": [1000, 150]})
    ...
    def test_basic_transaction_history_query(self, temp_catalog_properties):
        ...
        # Verify schema
        expected_columns = [
            "transaction_id", "commit_message", "start_time",
            "end_time", "status", "operation_count", "operation_types",
            "namespace_count", "table_count", "table_version_count",
            "stream_count", "partition_count", "delta_count"
        ]
    ...
    def test_transaction_state_filtering(self, temp_catalog_properties):
        ...
        with_running = dc.transactions(read_as=DatasetType.PANDAS, status_in=[TransactionStatus.SUCCESS, TransactionStatus.RUNNING])
        assert len(with_running) == base_count
        assert all(status in ["SUCCESS", "RUNNING"] for status in with_running["status"])
        
        # Test with FAILED status
        with_failed = dc.transactions(read_as=DatasetType.PANDAS, status_in=[TransactionStatus.SUCCESS, TransactionStatus.FAILED])
        ...
        with_paused = dc.transactions(read_as=DatasetType.PANDAS, status_in=[TransactionStatus.SUCCESS, TransactionStatus.PAUSED])
        assert len(with_paused) == base_count
        assert all(status in ["SUCCESS", "PAUSED"] for status in with_paused["status"])
        
        # Test all states
        all_states = dc.transactions(read_as=DatasetType.PANDAS, status_in=[TransactionStatus.SUCCESS, TransactionStatus.RUNNING, TransactionStatus.FAILED, TransactionStatus.PAUSED])
        assert len(all_states) == base_count
        
        # Test multiple states
        multi_state = dc.transactions(
            read_as=DatasetType.PANDAS, 
            status_in=[TransactionStatus.SUCCESS, TransactionStatus.RUNNING, TransactionStatus.FAILED, TransactionStatus.PAUSED]
        )
    ...
    def test_time_based_filtering(self, temp_catalog_properties):
        ...
        recent_txns = dc.transactions(
            read_as=DatasetType.PANDAS,
            start_time=mid_time
        )
        
        # Should get at least the transactions at or after that time
        assert len(recent_txns) >= 2, f"Expected at least 2 transactions after mid_time, got {len(recent_txns)}"
    ...
    def test_limit_and_pagination(self, temp_catalog_properties):
        ...
        assert limited_txns["transaction_id"].tolist() == all_txns["transaction_id"].head(2).tolist()
    ...
    def test_commit_message_functionality(self, temp_catalog_properties):
        ...
        result_messages = {
            row["transaction_id"]: row["commit_message"] 
        ...
        for txn_id, expected_msg in zip(created_txn_ids, test_messages):
            ...
            if expected_msg == "" and actual_msg is None:
                continue
            assert actual_msg == expected_msg, f"Commit message mismatch for {txn_id}: expected {expected_msg!r}, got {actual_msg!r}"
    ...
    def test_transaction_metadata_accuracy(self, temp_catalog_properties):
        ...
        for test_case in test_cases:
            ...
            assert metadata["operation_count"] > 0, f"Operation count should be > 0 for {txn_id}"
            assert metadata["operation_count"] >= metadata["table_count"], \
    ...
    def test_empty_catalog_graceful_handling(self, temp_catalog_properties):
        ...
        test_cases = [
            ...
            {"limit": 1, "status_in": [TransactionStatus.SUCCESS, TransactionStatus.RUNNING, TransactionStatus.FAILED, TransactionStatus.PAUSED]},
        ]
        ...
        for params in test_cases:
            ...
            assert list(result.columns) == expected_columns, f"Schema mismatch for params {params}"
    ...
    def test_error_handling_and_edge_cases(self, temp_catalog_properties):
        ...
        old_time_result = dc.transactions(
            read_as=DatasetType.PANDAS,
            start_time=1000000000  # Very old timestamp
        )
    ...
    def test_parameter_combinations(self, temp_catalog_properties):
        ...
        test_combinations = [
            ...
            # States + limit
            {"status_in": [TransactionStatus.SUCCESS, TransactionStatus.RUNNING, TransactionStatus.FAILED, TransactionStatus.PAUSED], "limit": 1},
            
            # Time + states
            {"start_time": time.time_ns() - 3600000000000, "status_in": [TransactionStatus.SUCCESS, TransactionStatus.RUNNING]},
...
class TestTransactionHistoryRegression:
    ...
    def test_sorting_consistency(self, temp_catalog_properties):
        ...
        assert result_times == expected_times, "Transactions not properly sorted by start_time descending"
    ...
    def test_function_signature_stability(self, temp_catalog_properties):
        ...
        assert actual_params == expected_params, f"Function signature changed: {actual_params}"
    ...
    def test_read_transaction(self, temp_catalog_properties):
        ...
        assert len(transaction_obj.operations) == 24 == result.iloc[0]["operation_count"]
        ...
        assert len(transaction_obj.operations) == 26 == result.iloc[1]["operation_count"]
