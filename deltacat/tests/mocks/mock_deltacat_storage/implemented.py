...
from deltacat.storage import ListResult, PartitionLocator
from deltacat.tests.test_utils.attribute_dict import recursive_attrdict
...
def list_namespaces(*args, **kwargs) -> ListResult[Namespace]:
    ...
    file_path = os.path.join(current_directory, "mock_responses", "list_deltas.json")

    f = open(file_path)
    deltas_list = json.load(f)
    f.close()
    return ListResult(recursive_attrdict(deltas_list))
