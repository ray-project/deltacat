...
@pytest.fixture
def valid_compact_partition_params():
...
def test_destination_partition_locator_is_optional():
...
def test_serialize_returns_json_string_with_all_fields(valid_compact_partition_params):
    from deltacat.compute.compactor.model.compact_partition_params import (
        CompactPartitionParams,
    )

    params = CompactPartitionParams.of(valid_compact_partition_params)
    serialized_params = params.serialize()
    assert isinstance(serialized_params, str)
    assert (
    ...
    assert (
        json.loads(serialized_params)["hash_bucket_count"] == params.hash_bucket_count
    )
    assert (
    ...
    assert (
        json.loads(serialized_params)["list_deltas_kwargs"] == params.list_deltas_kwargs
    )
...
def test_json_handler_for_compact_partition_params_serializes_set_to_list():
    from deltacat.compute.compactor.model.compact_partition_params import (
        CompactPartitionParams,
    )
    ...
    json_string = json.dumps(
        my_set, default=CompactPartitionParams.json_handler_for_compact_partition_params
    )
