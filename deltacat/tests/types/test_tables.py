...
def test_convert_to_pandas_error_cases():
    ...
    # Test None input
    with pytest.raises(ValueError, match="No pandas conversion function found for table type"):
        to_pandas(None)
...
def test_convert_to_arrow_error_cases():
    ...
    # Test None input
    with pytest.raises(ValueError, match="No pyarrow conversion function found for table type"):
        to_pyarrow(None)
...
def test_conversion_roundtrip_consistency():
    ...
    assert get_table_length(original_df) == get_table_length(roundtrip_df), "Row count should be preserved"
    assert list(original_df.columns) == list(roundtrip_df.columns), "Column names should be preserved"
