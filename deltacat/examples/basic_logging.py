...
from deltacat.examples.common.fixtures import create_runtime_environment, store_cli_args_in_os_environ
...
def logging_worker(var1, var2):
    ...
    print(f"Writing DEBUG log line from Worker to {DELTACAT_APP_LOG_DIR}: '{log_line_1}'")
    worker_logger.debug(log_line_1)

    log_line_2 = f"Worker Variable 1: {var1}"
    print(f"Writing INFO log line from Worker to {DELTACAT_APP_LOG_DIR}: '{log_line_2}'")
    worker_logger.info(log_line_2)

    log_line_3 = f"Worker Variable 2: {var2}"
    print(f"Writing INFO log line from Worker to {DELTACAT_APP_LOG_DIR}: '{log_line_3}'")
    worker_logger.info(log_line_3)


def run(var1="default1", var2="default2", **kwargs):
    log_line_1 = f"Driver Variable 1: {var1}"
    print(f"Writing INFO log line from Driver to {DELTACAT_APP_LOG_DIR}: '{log_line_1}'")
    driver_logger.info(log_line_1)

    log_line_2 = f"Driver Variable 2: {var2}"
    print(f"Writing INFO log line from Driver to {DELTACAT_APP_LOG_DIR}: '{log_line_2}'")
    driver_logger.info(log_line_2)

    print("Starting worker...")
    ray.get(logging_worker.remote(var1, var2))
    print(f"The driver is shutting down. Additional DeltaCAT system logs have been written to {DELTACAT_SYS_LOG_DIR}")
