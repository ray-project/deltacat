...
def inspect_catalog(
        ...
        for ns in namespaces.all_items():
            ...
            try:
                ...
                for table_def in tables.all_items():
                    ...
                    print(
                        f"       Schema: {table_version.schema.schema if table_version.schema else 'N/A'}"
                    )
                        ...
                        for stream in streams.all_items():
                            ...
                            try:
                                partitions = metastore.list_stream_partitions(
                                    stream=stream, catalog="default"
                                )

                                print(
                                    f"           📦 Partitions ({len(partitions.all_items())}):"
                                )
                                for i, partition in enumerate(
                                    partitions.all_items()[:3]
                                ):  # Show first 3
                                    print(
                                        f"             [{i+1}] ID: {partition.partition_id}"
                                    )
                                    print(
                                        f"                 Values: {partition.partition_values or 'None (unpartitioned)'}"
                                    )
                                    print(
                                        f"                 Stream Position: {partition.stream_position}"
                                    )

                                    # Get deltas for this partition
                                    try:
                                        deltas = metastore.list_partition_deltas(
                                            partition_like=partition, catalog="default"
                                        )
                                        print(
                                            f"                 🔄 Deltas: {len(deltas.all_items())}"
                                        )
                                        ...
                                        if max_stream_pos > 0:
                                            print(
                                                f"                 📈 Max Stream Position: {max_stream_pos}"
                                            )

                                    except Exception as e:
                                        print(
                                            f"                 ❌ Error listing deltas: {str(e)}"
                                        )

                                if len(partitions.all_items()) > 3:
                                    print(
                                        f"             ... and {len(partitions.all_items()) - 3} more partitions"
                                    )

                            except Exception as e:
                                print(
                                    f"           ❌ Error listing partitions: {str(e)}"
                                )
    ...
    print(
        "PYTHONPATH=/path/to/deltacat python deltacat/examples/compactor/compactor.py \\"
    )
...
Examples:
  ...
  python inspect.py --catalog-root /tmp/deltacat_test --namespace compactor_test --table events
    ...
    parser.add_argument(
        "--namespace", type=str, help="Filter by specific namespace (optional)"
    )

    parser.add_argument(
        "--table", type=str, help="Filter by specific table name (optional)"
    )
