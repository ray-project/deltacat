...
bypassing Beam's native Iceberg I/O limitations with positional deletes.
"""
...
class SparkSQLIcebergRead(beam.DoFn):
    ...
    This bypasses Beam's native Iceberg I/O and uses Spark's implementation directly,
    which correctly handles positional deletes created by DeltaCAT.
    """
    
    def __init__(self, table_name: str, catalog_uri: str = "http://localhost:8181", warehouse: str = "warehouse/"):
        """
    ...
    def setup(self):
        ...
        try:
            ...
            self.spark = (
                ...
                .appName(f"DeltaCAT Read - {self.table_name}")
    ...
    def process(self, element):
        ...
        try:
            ...
            print(f"📖 Reading table {self.table_name} using Spark SQL (handles positional deletes)")
...
class SparkSQLIcebergRewrite(beam.DoFn):
    ...
    that Beam can write to successfully.
    """
    ...
    def setup(self):
        ...
        try:
            ...
            self.spark = SparkSession.builder \
                .appName("IcebergRewrite") \
                .config("spark.jars.packages", f"org.apache.iceberg:iceberg-spark-runtime-{major_minor}_2.12:1.6.0") \
                .config("spark.sql.extensions", "org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions") \
            ...
            raise RuntimeError(f"PySpark is required for rewrite mode. Install with: pip install pyspark") from e
        except Exception as e:
    ...
    def process(self, element):
        ...
        try:
            print(f"📋 Rewriting table {self.table_name} to materialize positional deletes")
            ...
            print(f"   Rationale: Forces rewrite even with single positional delete file")
            ...
            if rewrite_result.rewritten_data_files_count > 0:
                print(f"✅ Successfully rewrote {rewrite_result.rewritten_data_files_count} data files")
                print(f"   - Added {rewrite_result.added_data_files_count} new data files")
