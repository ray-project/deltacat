...
Tail the INFO-level log events to see when the conversion job completes:
```bash
tail -f /tmp/deltacat/var/output/logs/deltacat-python.info.log
```
...
Options:
  ...
  --ray-inactivity-timeout FLOAT          Ray idle shutdown timeout in seconds (default: 10.0)
...
2. **Monitor Phase**:
   ...
   - **Always-On Deduplication**: Runs converter sessions on all tables with data (single or multi-snapshot)
