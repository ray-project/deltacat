...
def generate_complete_markdown_from_json(
    json_file: str, output_file: str = "corrected_comprehensive_matrix.md"
):
    ...
    arrow_type_descriptions = {
        ...
        "binary": "binary",
        "null": "null",
        "dictionary_string": "dictionary<values=string, indices=int32>",
        "int8": "int8",
        "int16": "int16",
        "uint8": "uint8",
        "uint16": "uint16",
        ...
        "list_int32": "list<element: int32>",
        "list_string": "list<element: string>",
    ...
    markdown = f"""### DeltaCAT Schema Inference Mappings

The tables below show the default DeltaCAT schema inference Arrow type
mappings across all supported dataset and content types. These type
mappings are generated by:

\n\n1. Creating a PyArrow table containing the given data types.
2. Resolving the target dataset types via `data = dc.from_pyarrow(pyarrow_table, target_dataset_type)`.
3. Writing to the table via `dc.write_to_table(data, table_name)`.
