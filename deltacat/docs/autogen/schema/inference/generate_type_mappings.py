...
def get_version_info():
    ...
    try:
        ...
        try:
            ...
            version_info['deltacat_version'] = pkg_resources.get_distribution('deltacat').version
        except:
...
def get_comprehensive_test_types() -> List[Tuple[str, str, List[Any]]]:
    ...
    return [
        ...
        ("large_string", "pa.large_string()", ["large hello", "large world", "large test"]),
        
        # Binary types
        ("binary", "pa.binary()", [b"hello", b"world", b"test"]),
        ("large_binary", "pa.large_binary()", [b"large hello", b"large world", b"large test"]),
        
        # Date and time types
        ("date32", "pa.date32()", [date(2023, 1, 1), date(2023, 12, 31), date(2024, 6, 15)]),
        ("date64", "pa.date64()", [date(2023, 1, 1), date(2023, 12, 31), date(2024, 6, 15)]),
        ("time32_s", "pa.time32('s')", [1754962113, 1754962114, 1754962115]),
        ("time32_ms", "pa.time32('ms')", [1754962113, 1754962114, 1754962115]),
        ("time64_us", "pa.time64('us')", [1754962113000000, 1754962114000000, 1754962115000000]),
        ("time64_ns", "pa.time64('ns')", [1754962113000000000, 1754962114000000000, 1754962115000000000]),
        ("timestamp_us", "pa.timestamp('us')", [datetime(2023, 1, 1, 12, 0, 0), datetime(2023, 12, 31, 23, 59, 59), datetime(2024, 6, 15, 10, 30, 45)]),
        ("timestamp_s_utc", "pa.timestamp('s', tz='UTC')", [datetime(2023, 1, 1, 12, 0, 0), datetime(2023, 12, 31, 23, 59, 59), datetime(2024, 6, 15, 10, 30, 45)]),
        ("duration_s", "pa.duration('s')", [1754962113, 1754962114, 1754962115]),
        ("duration_ms", "pa.duration('ms')", [1754962113000, 1754962114000, 1754962115000]),
        ("duration_us", "pa.duration('us')", [1754962113000000, 1754962114000000, 1754962115000000]),
        ("duration_ns", "pa.duration('ns')", [1754962113000000000, 1754962114000000000, 1754962115000000000]),
        ("month_day_nano", "pa.month_day_nano_interval()", [pa.scalar((1, 15, -30), type=pa.month_day_nano_interval()), pa.scalar((2, 15, -30), type=pa.month_day_nano_interval()), pa.scalar((3, 15, -30), type=pa.month_day_nano_interval())]),

        # Decimal
        ("decimal128_5_2", "pa.decimal128(5, 2)", [Decimal("123.45"), Decimal("-67.89"), Decimal("999.99")]),
        ("decimal256_76_38", "pa.decimal256(76, 38)", [Decimal("123.45"), Decimal("-67.89"), Decimal("999.99")]),
        ...
        # Struct type
        ("struct_simple", "pa.struct([('name', pa.string()), ('age', pa.int32())])", [{'name': 'Alice', 'age': 30}, {'name': 'Bob', 'age': 25}, {'name': 'Charlie', 'age': 35}]),
        
        # Dictionary type  
        ("dictionary_string", "pa.dictionary(pa.int32(), pa.string())", ["apple", "banana", "apple"]),

        # Map type
        ("map_string_int32", "pa.map_(pa.string(), pa.int32())", [{"a": 1, "b": 2}, {"c": 3, "d": 4}, {"e": 5}]),
    ]


def extract_file_paths_from_deltas(all_objects: List[Any]) -> List[str]:
    """Extract file paths from Delta objects using proper manifest parsing."""
    file_paths = []
            ...
            if hasattr(delta_obj, 'manifest') and delta_obj.manifest:
                ...
                if hasattr(manifest, 'entries') and manifest.entries:
                    for entry in manifest.entries:
                        if hasattr(entry, 'get') and 'url' in entry:
                            file_url = entry['url']
                        elif hasattr(entry, 'get') and 'uri' in entry:
                            file_url = entry['uri']
                        else:
                            continue
...
def inspect_specific_file_physical_schema(file_path: str, content_type: ContentType) -> Dict[str, Any]:
    ...
    try:
        ...
        if content_type == ContentType.PARQUET:
            ...
            print(f"Parquet schema: {parquet_schema_string}")
            column_info = {}
            parquet_col_index = 0
            ...
            for i in range(len(arrow_schema)):
                ...
                if col.max_definition_level > 1 or col.max_repetition_level > 1:
                    ...
                    parquet_physical_type_name_prefix = f"{col.max_definition_level}-Level" if col.max_definition_level > 0 else ""
                    parquet_physical_type_name = f"{parquet_physical_type_name_prefix} {parquet_physical_type_name_suffix}"
                    parquet_logical_type_name = "LIST" if "(List)" in parquet_schema_string else "MAP" if "(Map)" in parquet_schema_string else ""
                    ...
                    if 'list<' in arrow_type_str.lower():
                        ...
                        struct_fields = arrow_type_str.count(',') + 1 if ',' in arrow_type_str else 1
                    ...
                    try:
                        ...
                        column_info[f"column_{i}"] = {
                            ...
                            'parquet_logical_type': str(col.logical_type) if col.logical_type else None,
                            'parquet_converted_type': str(col.converted_type) if col.converted_type else None,
                            'nullable': arrow_field.nullable
                        }
                        parquet_col_index += 1
            ...
            for i, field in enumerate(orc_file.schema):
                column_info[f"column_{i}"] = {
                    'arrow_type': str(field.type),
                    'orc_type_kind': str(field.type).split('(')[0] if '(' in str(field.type) else str(field.type),
                    'nullable': field.nullable
                }
...
def run_single_test_with_proper_file_mapping(
    arrow_type_name: str, arrow_type_code: str, test_data: List[Any], 
    dataset_type: DatasetType, content_type: ContentType, 
    ...
    try:
        ...
        arrow_table = pa.Table.from_arrays([pa.array(test_data, type=arrow_type)], names=[arrow_type_name])
        ...
        try:
            ...
            if file_paths:
                ...
                physical_schema = inspect_specific_file_physical_schema(file_path, content_type)
            else:
        ...
        return {
            ...
            'read_back_type': str(read_result.schema.field(0).type) if hasattr(read_result, 'schema') else 'unknown',
            'physical_schema': physical_schema,
            'type_preserved': str(arrow_type) == str(read_result.schema.field(0).type) if hasattr(read_result, 'schema') else False,
            'error': None,
        ...
        return {
            ...
            'content_type': content_type.value,
            'success': False,
            'original_arrow_type': str(eval(arrow_type_code)) if 'eval' not in str(e) else 'unknown',
            'physical_schema': {},
            'error': str(e),
...
def main():
    ...
    dataset_types = [DatasetType.PYARROW, DatasetType.PANDAS, DatasetType.POLARS, DatasetType.DAFT, DatasetType.RAY_DATASET]  # All dataset types
    content_types = [ContentType.PARQUET, ContentType.FEATHER, ContentType.AVRO, ContentType.ORC]  # Test 4 content types
    
    print(f"Testing {len(arrow_types)} Arrow types × {len(dataset_types)} dataset types × {len(content_types)} content types")
    print()
    ...
    for arrow_type_name, arrow_type_code, test_data in arrow_types:
        ...
        for dataset_type in dataset_types:
            for content_type in content_types:
                test_count += 1
                print(f"  [{test_count:2d}/{total_tests}] {dataset_type.value} → {content_type.value}")
                
                result = run_single_test_with_proper_file_mapping(
                    arrow_type_name, arrow_type_code, test_data,
                    dataset_type, content_type, catalog_name
                )
                
                if result['success']:
                    if result['physical_schema'].get('error'):
                        print(f"    ❌ Physical schema error: {result['physical_schema']['error']}")
                    else:
                        ...
                        if columns:
                            ...
                            if content_type == ContentType.PARQUET:
                                ...
                                physical_type = first_col.get('feather_preserved_type', 'unknown')
                                print(f"    ✅ Physical type: {physical_type}")
                            elif content_type == ContentType.AVRO:
                                physical_type = first_col.get('avro_type', 'unknown')
                                print(f"    ✅ Physical type: {physical_type}")
                            elif content_type == ContentType.ORC:
                                physical_type = first_col.get('orc_type_kind', 'unknown')
                                print(f"    ✅ Physical type: {physical_type}")
                        else:
